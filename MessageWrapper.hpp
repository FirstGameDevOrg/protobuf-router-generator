...
class CommandRouterMap final
    : public std::map<std::string, CommandRouter>
{

public:
    using std::map<std::string, CommandRouter>::operator=;
    bool parse(const std::string& content)
    {
        try {
            *this = nlohmann::json::parse(content);
            return true;
        }
    ...
    void append(const std::string& key, const CommandRouter& value)
    {
        std::map<std::string, CommandRouter>::insert_or_assign(key, value);
    }
    
    void append(const std::string& key, int16_t high, int16_t low)
    {
        append(key, CommandRouter{ high, low });
    }
